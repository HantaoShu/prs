                        ...
                        for i, (train_ind, test_ind) in (enumerate(kf.split(X, y))):
                            ...
                            for epoch in tqdm(range(n_epoch)):
                                ...
                                for xx, yy in dataloader:
                                    ...
                                    loss = ((out-yy)**2).sum()#F.binary_cross_entropy_with_logits(out, yy)
                                ...
                                if epoch % 1 == 0:
                                    with torch.no_grad():
                                        ...
                                        aucs.append([args.feature, sample, l0, l1, lgraph, layer, i, epoch,
                                                     pearsonr(y_test.cpu().detach().numpy(), out.cpu().detach().numpy())[0]])
                                        out = model(X_val, L, edge_weight)[:, 0]
                                        aucs[-1].append(pearsonr(y_val.cpu().detach().numpy(), out.cpu().detach().numpy())[0])

                                        out = model(X_train, L, edge_weight)[:, 0]
                                        aucs[-1].append(pearsonr(y_train.cpu().detach().numpy(), out.cpu().detach().numpy())[0])
